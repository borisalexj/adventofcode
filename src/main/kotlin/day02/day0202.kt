...
//   val parsed = parseInput(sampleInput02)
//    println(parsed)
//
//    val passedGames = checkGames(parsed, sampleSet)
//    println(passedGames)
//    println(passedGames.sumOf { it.id })

    val parsed = parseInput(realInput)
    println(parsed)

    val passedGames = checkGames0201(parsed, realset)
    println(passedGames)
    println("result 0201 - " + passedGames.sumOf { it.id })
    val maxSets = checkGames0202(parsed)
    println(maxSets)
    println("result 0202 - " +calculate0202(maxSets))

}

fun calculate0202(maxSets: java.util.ArrayList<CubeSet>) : Int {
    var result = 0
    for (set in maxSets) {
        result = result + set.blue*set.red*set.green
    }
    return result
}

fun checkGames0202(parsed: java.util.ArrayList<Game>): ArrayList<CubeSet>  {
    val result = arrayListOf<CubeSet>()


    for (game in parsed) {
        val cubeSet = CubeSet()
        for (set in game.sets) {
            if (cubeSet.red < set.red) {
                cubeSet.red = set.red
            }
            if (cubeSet.green < set.green) {
                cubeSet.green = set.green
            }
            if (cubeSet.blue < set.blue) {
                cubeSet.blue = set.blue
            }
        }
        result.add(cubeSet)
    }
...
fun checkGames_(parsed: java.util.ArrayList<Game>, sampleSet: CubeSet): ArrayList<Game>  {
    val result = arrayListOf<Game>()

    for (game in parsed) {
        var passed = true
        for (set in game.sets) {
            if (set.red > sampleSet.red  || set.blue > sampleSet.blue || set.green > sampleSet.green) {
                passed = false
            }
        }
        if (passed) {
...
fun parseInput_(input: List<String>) : ArrayList<Game> {
    val result = arrayListOf<Game>()

    for (line in input) {
        val game = Game()
        val id = line.split(":")[0].filter { it.isDigit() }
        game.id = id.toInt()
        val sets = line.split(":")[1].split(";")
        var setsProcessed = arrayListOf<CubeSet>()
        for (s in sets) {
            val set = CubeSet()
            for (cubes in s.split(",")) {
                if (cubes.contains("red")) {
                     set.red = cubes.filter { it.isDigit() }.toInt()
                }
                if (cubes.contains("blue")) {
                     set.blue = cubes.filter { it.isDigit() }.toInt()
                }
                if (cubes.contains("green")) {
                     set.green = cubes.filter { it.isDigit() }.toInt()
                }
            }
            game.sets.add(set)
...
//val sampleCheck02 = "8 green, 6 blue, 20 red"
//
//val sampleSet = CubeSet().also {
//    it.red = 12
//    it.green = 13
//    it.blue = 14
//}
//
//val realset = CubeSet().also {
//    it.red = 12
//    it.green = 13
//    it.blue = 14
//}
...
//class Game(){
//    var id : Int = 0
//    var sets: ArrayList<CubeSet> = arrayListOf<CubeSet>()
