...
//    val expected = day19real2
    ...
    expected.forEach { pattern ->
        ...
        while (true) {
            for (i in maxTowel downTo 1) {
                val toSearch = pattern.subSequence(letter, letter + min(i, pattern.length-letter)).toString()
                if (towels.contains(toSearch)) {
                    res.add(toSearch)
                    letter = letter +i
                    break
                }
                if (i == 1 && !towels.contains(toSearch)) {
                    res.add("impossible")
                    letter = letter +i
                    break
                }
            }
            if (letter >= pattern.length || res.contains("impossible"))
                break
