...
//
        for (i in 0..coordinatePairs.size - 1) {
            for (c in 1..coordinatePairs.size - 1) {
                ...
                val a1 = Pair(
                    if (a1DeltaY < 0) {
                        min(coord1.first, coord2.first) + a1DeltaY
                    } else {
                        max(coord1.first, coord2.first) + a1DeltaY
                    },
                    if (a1DeltaX < 0) {
                        min(coord1.second, coord2.second) + a1DeltaX
                    } else {
                        max(coord1.second, coord2.second) + a1DeltaX
                    }
                )
                antiCoordinates.add(a1)
                val a2 = Pair(
                    if (a1DeltaY <0) {
                        max(coord1.first, coord2.first) + abs(a1DeltaY)}
                    else {
                        min(coord1.first, coord2.first) - a1DeltaY
                    },
                    if (a1DeltaX <0) {
                        max(coord1.second, coord2.second) + abs(a1DeltaX) }
                    else {
                        min(coord1.second, coord2.second) - a1DeltaX
                    })
//                Any()
                antiCoordinates.add(a2)
//                println("$a1 $a2")
            }
        }
    }
