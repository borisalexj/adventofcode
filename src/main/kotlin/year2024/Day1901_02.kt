...
//        val res = arrayListOf<String>()
        recursiveSearch(result, arrayListOf(), letter, pattern, towels, expected)
...
fun recursiveSearch(
results: HashMap<String, ArrayList<String>>,
...
//        println("success")
        ...
        results[pattern] = newLocalResults
        return
    }
    for (i in 0..pattern.length-1) {
        val twls =
        towels.filter {
            it == pattern.subSequence(letter, min(pattern.length-i, letter + it.length))
         }
         if (twls.isEmpty()) { return }
            twls.forEach {
             val newLocalResults =  arrayListOf<String>()
             newLocalResults.addAll(localResult)
             newLocalResults.add(it)
             recursiveSearch(results, newLocalResults, letter + it.length, pattern, towels, expected)
          }
