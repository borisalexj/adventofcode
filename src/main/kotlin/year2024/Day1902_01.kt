...
//    val expected = day19real2
    ...
    val result = hashMapOf<String, ArrayList<ArrayList<String>>>()
    val maxTowel = towels.map {it.length}.max()

//    expected.take(1).forEach { pattern ->
    expected.forEach { pattern ->
        var letter = 0
        println(pattern)
//        val results = arrayListOf<ArrayList<String>>()
//        val res = arrayListOf<String>()
        recursiveSearch2(result, arrayListOf(), letter, pattern, towels, expected)
...
//        result[pattern] = res
    ...
    println("----------------")
    println("Result 2:")
    result.values.flatten().size.let { println(it) }
//    println(result)
    result
//        .filter { !it.value.contains("impossible") }
        .keys.map {
        println("$it - ${result[it]}")
     }.size.let { println(it) }
}

fun recursiveSearch2(
results: HashMap<String, ArrayList<ArrayList<String>>>,
localResult: ArrayList<String>,
letter: Int,
pattern: String,
towels: MutableList<String>,
expected: java.util.ArrayList<String>
) {
//    println(localResult.joinToString (""))
//    if (results.getOrDefault(pattern, null) != null) {return}
    if (localResult.joinToString("") == pattern) {
//        println("success")
         val newLocalResults =  arrayListOf<String>()
         newLocalResults.addAll(localResult)
        var res =  results.getOrDefault(pattern, null)
        if (res == null) {
            res = arrayListOf()
            res.add(newLocalResults)
        } else {
            res.add(newLocalResults)
        }
        results[pattern] = res
    ...
    if (letter >= pattern.length) return
    for (i in 0..pattern.length-1) {
        val twls =
        towels.filter{ it.length ==i }.filter {
//            println("$pattern - $i - $it - $letter - ${letter + it.length-i} - ${pattern.length - 1}")
//            println(pattern.subSequence(letter, min(letter + it.length, pattern.length )))
            it == pattern.subSequence(letter, min(letter + it.length, pattern.length))
         }
         if (twls.isEmpty()) { continue }
            twls.forEach {
//                println(it)
             val newLocalResults =  arrayListOf<String>()
             newLocalResults.addAll(localResult)
             newLocalResults.add(it)
             recursiveSearch2(results, newLocalResults, letter + it.length, pattern, towels, expected)
          }

    }
}

//val day19sample1 = "r, wr, b, g, bwu, rb, gb, br"
//val day19sample2 = arrayListOf(
//
//                   "brwrr",
//                   "bggr",
//                   "gbbr",
