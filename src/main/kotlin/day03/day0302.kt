...
fun main(args: Array<String>) {
    val input = realinput0301
//    val input = sampleinput0301 // realinput0301
    val stars = ArrayList<Pair<Int, Int>>()
    
//    val stars = HashMap<String, String>

    val numbers = ArrayList<Triple<String,Int,Int>>()
    ...
    input.forEachIndexed { indexY, line ->
        ...
        line.forEachIndexed { indexX, symbol ->
            if (!numberStarted && symbol.isDigit()) {
                numberStarted = true
...
//            println("$symbol - $digitIsOk")
            if (numberStarted && !symbol.isDigit() && digitIsOk) {
                println("$digitIsOk - $currentNumber")
                numbers.add(Triple(currentNumber,starX, starY))
                stars.add(Pair(starX, starY))
                currentNumber = ""
                numberStarted = false
                digitIsOk = false
                        starX = -1
...
//            if (symbol.equals('.') && digitIsOk) {
//                digitIsOk = false
//            }
        }
                    if (numberStarted && digitIsOk) {
                        println("$digitIsOk - $currentNumber")
                        numbers.add(Triple(currentNumber,starX, starY))
                        stars.add(Pair(starX, starY))
                        currentNumber = ""
...
//        println("-------------------")
    ...
    println("stars - " + stars.toSet())
    println("stars.size - " + stars.toSet().size)
...
//    stars.toSet().forEach { star ->
////        println(numbers.filter {
////            it.second == star.first && it.third == star.second }.takeIf { it.size>1 }?.filterNotNull())
////
//        numbers.filter {
//            it.second == star.first && it.third == star.second }.takeIf { it.size>1 }?.forEach {
////                println("-- " + it)
//                res.add(it)
//             }
//     }
...
//    println("*************")
    ...
    var result = 0L
     ...
     stars.toSet().forEach { star ->
         numbers.filter {
             it.second == star.first && it.third == star.second
          }.takeIf { it.isNotEmpty() }.let {
              if (it != null && it.size >1) {
                  var middleResult = 1
                  it.forEach {
                      println(it.toString() + " " + it.first.toString())
                    middleResult = middleResult * it.first.toInt()

                  }
                println("middleResult - $middleResult")
