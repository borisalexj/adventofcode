...
//                currentNumber = currentNumber + symbol
            ...
            if (symbol.isDigit() && checkForSymbol(indexX, indexY, input)) {
                digitIsOk = true
            }
...
//            }
    ...
    println("result = " + numbers.map { it.toInt() }.sum())
}

fun checkForSymbol(indexX: Int, indexY: Int, input: List<String>) : Boolean {
    val isOk = false

    if (
    checkSymbolValidity(input.getOrNull(indexY-1)?.getOrNull(indexX-1)) ||
    checkSymbolValidity(input.getOrNull(indexY-1)?.getOrNull(indexX)) ||
    checkSymbolValidity(input.getOrNull(indexY-1)?.getOrNull(indexX+1)) ||

    checkSymbolValidity(input.getOrNull(indexY)?.getOrNull(indexX-1)) ||
    checkSymbolValidity(input.getOrNull(indexY)?.getOrNull(indexX+1)) ||
...
fun checkSymbolValidity(s: Char?) : Boolean {
